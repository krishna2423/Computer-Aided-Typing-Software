...
def pick(paragraphs, select, k):
    ...
    ''
    """
    new = [paragraph for paragraph in paragraphs if select(paragraph) == True]
    if k < len(new): 
        return new[k]
    else: 
        return ''
...
def about(subject):
    ...
    def paragraph_checker(paragraph): 
        revised = split(remove_punctuation(lower(paragraph)))
        revised_subject = [remove_punctuation(lower(elem)) for elem in subject ]
        for elem in revised: 
            for topic in revised_subject: 
                if topic == elem: 
                    return True
        return False
...
def accuracy(typed, source):
    ...
    if len(typed_words) != 0 and len(source_words) == 0: 
        return 0.0
    if len(typed_words) < len(source_words) and typed_words[0] == source_words[0] : 
        return 100.0
    if len(typed_words) < len(source_words) and typed_words[0] != source_words[0] : 
        return 0.0
    count = 0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    'testing'
    """
    closest = min(word_list, key= lambda x: diff_function(typed_word, x, limit))
    if typed_word in word_list: 
        return typed_word
    elif diff_function(typed_word, closest, limit)> limit:
        return typed_word
    else: 
        return closest
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    if typed == source or len(typed) == 0 or len(source) == 0: 
        return abs(len(source) - len(typed))
...
def time_per_word(words, timestamps_per_player):
    ...
    times = []
    for lst in tpp: 
        times.append([lst[i+1] - lst[i] for i in range(len(lst) - 1)])
...
def fastest_words(words_and_times):
    ...
    masterlst = [[] for _ in player_indices]
    for i in word_indices: 
        fastest = 0
        min_time = get_time(times, 0, i)
        for k in player_indices: 
            if get_time(times, k, i) < min_time: 
                min_time = get_time(times, k, i)
                fastest = k
        masterlst[fastest].append(words[i])
    return masterlst
